...
> `entlong` 仅在原世界的生命周期内有效：重用世界 ID 可能使旧句柄重新有效。如有需要，请将 `EcsWorld.InstanceID` 与句柄一同保存，并在目标世界中解包前进行比较。
...
> 不支持在组件的 `OnAdd`/`OnDel` 中对同一池进行结构性修改，也不支持在自定义 `Copy` 中修改任一参与池的结构：组件引用可能失效。请将修改加入队列，在回调结束后执行。DEBUG 仅对每个池警告一次，不会阻止操作或保证其安全，Stability Mode 也不例外。
...
## 世界组件
使用世界组件可以将额外的数据附加到世界上。世界组件可以使用 `struct` 或 `class` 类型。`Get` 返回对存储值的引用。

引用类型的世界组件初始值为 `null`，必须由调用者赋值。`IEcsWorldComponent<T>` 生命周期回调仅支持 `struct` 组件。如果引用类型实现了此接口，注册时会输出警告，并忽略其生命周期回调。

同一组件类型的存储由所有世界共享。在任意世界注册该类型后，应重新获取 `ref`，因为扩容可能替换底层数组。释放组件或销毁世界后，不得继续使用该引用。传递给 `Init` 和 `OnDestroy` 的引用在嵌套注册期间保持有效：最外层的同类型回调结束时会合并扩容产生的数组，包括异常退出的情况。跨线程使用外部引用时，必须协调同类型组件的注册；获取操作的线程安全不代表返回的引用永久有效。
